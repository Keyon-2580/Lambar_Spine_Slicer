...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for data in (test_loader):
            ...
            nib.save(array_img, '/home/sxchongya/unet_pytorch/fixed/mask_%d.nii.gz' )


if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    path = '/home/sxchongya/original_file/temporary/volume-70.nii.gz'
    test_set = DataSet(4, 0.5, path)
    test_loader = DataLoader(dataset=test_set, batch_size=1, num_workers=1, shuffle=False)
    model = torch.load('output/model-625-1.pth').to(device)
    print(model)
